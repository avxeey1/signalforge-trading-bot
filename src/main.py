...
def extract_token_address(text: str) -> str | None:
    """Extract Solana token address from text using regex."""
    match = re.findall(r"\b[1-9A-HJ-NP-Za-km-z]{32,44}\b", text)
    return match[-1] if match else None


def truncate_address(addr: str | None, chars: int = 8) -> str:
...
def add_trade(token: str, amount: float, price: float | None = None, ret: float | None = None) -> None:
    ...
    logger.info(f"Trade recorded: {token} | Amount: {amount} SOL | Price: {price}")
...
async def balance_handler(event):
    ...
    try:
        ...
        await event.reply("❌ Error fetching balance.")
...
async def history_handler(event):
    ...
    try:
        ...
        await event.reply("❌ Error fetching history.")
...
async def pnl_handler(event):
    ...
    try:
        ...
        logger.error(f"Error in pnl_handler: {e}")
        await event.reply("❌ Error calculating PnL.")
...
async def wallet_handler(event):
    ...
    try:
        ...
        logger.error(f"Error in wallet_handler: {e}")
        await event.reply("❌ Error retrieving wallet.")


@client.on(events.NewMessage(pattern="/runbot"))
async def runbot_handler(event):
    """Handle /runbot command."""
    global bot_status
    try:
        bot_status = "running"
        ...
        logger.error(f"Error in runbot_handler: {e}")
...
async def stopbot_handler(event):
    ...
    try:
        bot_status = "stopped"
        logger.info("Bot status changed to stopped")
        await event.reply("🛑 Bot stopped.")
    except Exception as e:
        logger.error(f"Error in stopbot_handler: {e}")
...
async def about_handler(event):
    ...
    try:
        ...
        await event.reply("❌ Error retrieving bot info.")
    ...
    try:
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"SOL sent: {amount} to {truncate_address(addr)}")
            else:
                await event.reply(f"❌ Failed: {tx}")
                logger.error(f"Failed to send SOL: {tx}")
        except ValueError:
            await event.reply("❌ Invalid amount.")
    except Exception as e:
        logger.error(f"Error in send_handler: {e}")
        await event.reply("❌ Error processing send.")
...
async def receive_handler(event):
    ...
    try:
        ...
        logger.error(f"Error in receive_handler: {e}")
...
async def channel_handler(event):
    """Monitor channel for trading signals."""
    try:
...
async def main() -> None:
    ...
    try:
        ...
        logger.info("🚀 SignalForge Bot starting...")
        logger.info(f"📡 Monitoring channel: {CHANNEL}")
        logger.info(f"💰 Trade amount: {TRADE_AMOUNT} SOL")
